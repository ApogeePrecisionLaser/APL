...
public class OrgDetailEntryController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String office = request.getParameter("task");
        List list = null;
        String data = null;
        String searchOrganisation = "";
        if (request.getParameter("str") != null) {
         
            input = request.getParameter("str").trim();
...
//            if (request.getParameter("task") != null || !(request.getParameter("Search").isEmpty())) {
           
                String jSONFormate = "";
                if (request.getParameter("task").equals("Search")) {
                    String org_office_id = request.getParameter("org_office");
                   String org = request.getParameter("searchOrganisation");
                       
                       String organisation_id = String.valueOf(createOrganModel.getOrganisation_id(org));
                    if (organisation_id != null && !organisation_id.isEmpty() && org_office_id != null && !org_office_id.isEmpty()) {
                         jSONFormate =  createOrganModel.getOfficePersonDetail(Integer.parseInt(organisation_id.trim()), Integer.parseInt(org_office_id.trim()));
                      out.println(jSONFormate);
                       JSONObject gson = new JSONObject();
                     gson.put("items",jSONFormate);
                   out.println(gson);
                        

                    }
                } 
                else if (request.getParameter("task").equals("Search")) {
                     String org = request.getParameter("searchOrganisation");
        ...
        if (task != null) {
            if (task.equals("CreateOrganisation")) {
                ...
                if (request.getParameter("organisation") != null && !request.getParameter("organisation").isEmpty()) {
                    createOrgn.setOrganisation(request.getParameter("organisation").trim()); 
                      createOrgn.setOrganisation_id(createOrganModel.getOrganisation_id(request.getParameter("organisation")));
                }
                ...
                createOrgn.setAge((request.getParameter("age")));
